...
def load_model(model_type):
    ...
    data = load_data()
    name, modeltype = data["model"][model_type]["name"], data["model"][model_type]["type"]
...
def gen_caption(image, caption_type, length_penalty, repetition_penalty, temperature):
    ...
    if caption_type == "Beam Search":
        caption = model.generate({"image": image},length_penalty=length_penalty, repetition_penalty=repetition_penalty)
    else:
        caption = model.generate(
            {"image": image}, use_nucleus_sampling=True, num_captions=3, repetition_penalty=repetition_penalty, temperature=temperature
        )
...
def on_ui_tabs():
    ...
    with gr.Blocks(analytics_enabled=False) as blip2:
        with gr.Column():
            ...
            with gr.Row():
                with gr.Column():
                    caption_type = gr.Radio(choices=["Beam Search", "Nucleus Sampling"], label='Type', value="Beam Search", interactive=True)
                    with gr.Tab("Beam Search"):
                        length_penalty = gr.Slider(
                            label="Length Penalty", minimum=-1, maximum=2, value=1, interactive=True)
                        repetition_penalty = gr.Slider(
                            label="Repeat Penalty", minimum=1, maximum=5, value=1.5, interactive=True)
                    with gr.Tab("Nucleus Sampling"):
                        temperature = gr.Slider(
                            label="Temperature", minimum=0.5, maximum=1, value=1, interactive=True)
                        repetition_penalty = gr.Slider(
                            label="Repeat Penalty", minimum=1, maximum=5, value=1.5, interactive=True)
            ...
            with gr.Row():
                send_to_buttons = modules.generation_parameters_copypaste.create_buttons(["txt2img", "img2img", "extras"])

        modules.generation_parameters_copypaste.bind_buttons(send_to_buttons, "", output_text)

        model_type.change(load_model, inputs=[model_type])

        btn_caption.click(
            gen_caption, inputs=[input_image, caption_type, length_penalty, repetition_penalty, temperature], outputs=[output_text]
        )
